import { Result, fromErr, fromOk, Opaque, Maybe } from "elytra-ts";
...
export type FieldInternal<E, V, T> = {
  ...
  _memo: Maybe<Result<E, T>>;
};

export type ParseDontValidateFn<E, V, T> = (s: V) => Result<E, T>;
...
export function error<E, V, T>(f: Field<E, V, T>): Maybe<E> {
  const fi = _internal(f);
  return fi._memo == null ? null : fromErr(fi._memo);
}

export function value<E, V, T>(f: Field<E, V, T>): Maybe<T> {
  const fi = _internal(f);
  return fi._memo == null ? null : fromOk(fi._memo);
}
