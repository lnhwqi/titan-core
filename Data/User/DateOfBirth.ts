...
import { Opaque, Result, fromOk, err, ok, Maybe } from "elytra-ts";
...
export function createDateOfBirth(
  ...
  return fromOk(createDateOfBirthE(year, month, day));
}
...
export const dateOfBirthDecoder: JD.Decoder<DateOfBirth> = JD.string.transform(
  (s) => {
    ...
    const dateOfBirth = createDateOfBirth(year, month, day);
    if (dateOfBirth == null) {
      throw new Error(`Invalid DateOfBirth: ${s}`);
    }
    return dateOfBirth;
  }
);
